...
<template>
	<ElForm class="query-form" :ref="(el: any) => { queryForm.ref = el }" :model="queryCommon.data" :inline="true"
		...
		<ElFormItem prop="cornCode">
			<ElInput v-model="queryCommon.data.cornCode" :placeholder="t('common.name.platform.corn.cornCode')" :clearable="true" />
		</ElFormItem>
		<ElFormItem prop="cornName">
			<ElInput v-model="queryCommon.data.cornName" :placeholder="t('common.name.platform.corn.cornName')" :clearable="true" />
		</ElFormItem>
