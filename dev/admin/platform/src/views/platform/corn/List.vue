...
const table = reactive({
	...
	{
		dataKey: 'cornCode',
		title: t('common.name.platform.corn.cornCode'),
		key: 'cornCode',
		width: 150,
		align: 'center',
	},
	{
		dataKey: 'cornName',
		title: t('common.name.platform.corn.cornName'),
		key: 'cornName',
		width: 150,
		align: 'center',
	},
	{
		dataKey: 'cornPattern',
		title: t('common.name.platform.corn.cornPattern'),
		key: 'cornPattern',
		width: 150,
		align: 'center',
	},
...
const handleEditCopy = (id: number, type: string = 'edit') => {
	request('/platform/corn/info', { id: id }).then((res) => {
...
const handleDelete = (idArr: number[]) => {
	ElMessageBox.confirm('', {
		...
		request('/platform/corn/del', { idArr: idArr }, true).then((res) => {
			getList()
		}).catch(() => { })
	}).catch(() => { })
}
...
const getList = async (resetPage: boolean = false) => {
	...
	table.loading = true
	try {
		const res = await request('/platform/corn/list', param)
