		...
		saveForm.ref.validate(async (valid: boolean) => {
			...
			try {
				if (param?.idArr?.length > 0) {
					await request('/platform/corn/update', param, true)
				} else {
					await request('/platform/corn/create', param, true)
				}
	...
	<ElDrawer class="save-drawer" :ref="(el: any) => { saveDrawer.ref = el }" v-model="saveCommon.visible"
		...
		<ElScrollbar>
			<ElForm :ref="(el: any) => { saveForm.ref = el }" :model="saveForm.data" :rules="saveForm.rules"
				...
				<ElFormItem :label="t('platform.corn.name.cornCode')" prop="cornCode">
					<ElInput v-model="saveForm.data.cornCode" :placeholder="t('platform.corn.name.cornCode')" minlength="1" maxlength="30" :show-word-limit="true" :clearable="true" />
				</ElFormItem>
				<ElFormItem :label="t('platform.corn.name.cornName')" prop="cornName">
					<ElInput v-model="saveForm.data.cornName" :placeholder="t('platform.corn.name.cornName')" minlength="1" maxlength="30" :show-word-limit="true" :clearable="true" />
				</ElFormItem>
				<ElFormItem :label="t('platform.corn.name.cornPattern')" prop="cornPattern">
					<ElInput v-model="saveForm.data.cornPattern" :placeholder="t('platform.corn.name.cornPattern')" minlength="1" maxlength="30" :show-word-limit="true" :clearable="true" />
				</ElFormItem>
