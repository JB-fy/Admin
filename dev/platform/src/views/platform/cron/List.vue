...
const table = reactive({
	...
	{
		dataKey: 'cronName',
		title: t('platform.cron.name.cronName'),
		key: 'cronName',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'cronCode',
		title: t('platform.cron.name.cronCode'),
		key: 'cronCode',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'cronPattern',
		title: t('platform.cron.name.cronPattern'),
		key: 'cronPattern',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'remark',
		title: t('platform.cron.name.remark'),
		key: 'remark',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'isStop',
		title: t('platform.cron.name.isStop'),
		key: 'isStop',
		align: 'center',
		width: 100,
		cellRenderer: (props: any): any => {
...
const handleEditCopy = (id: number, type: string = 'edit') => {
	request('/platform/cron/info', { id: id }).then((res) => {
...
const handleDelete = (idArr: number[]) => {
	ElMessageBox.confirm('', {
		...
		request('/platform/cron/del', { idArr: idArr }, true).then((res) => {
			getList()
		}).catch(() => { })
	}).catch(() => { })
}
...
const getList = async (resetPage: boolean = false) => {
	...
	table.loading = true
	try {
		const res = await request('/platform/cron/list', param)
