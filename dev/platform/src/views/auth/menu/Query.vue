    ...
    <ElForm class="query-form" :ref="(el: any) => { queryForm.ref = el }" :model="queryCommon.data" :inline="true"
        ...
        <ElFormItem prop="pid">
            <MyCascader v-model="queryCommon.data.pid" :placeholder="t('auth.menu.name.pid')"
                :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/auth/menu/tree' }" :defaultOptions="[{ id: 0, label: t('common.name.allTopLevel') }]"
        ...
        <ElFormItem prop="isStop" style="width: 100px;">
            <ElSelectV2 v-model="queryCommon.data.isStop" :options="tm('common.status.whether')"
                :placeholder="t('auth.menu.name.isStop')" :clearable="true" />
        </ElFormItem>
        <ElFormItem>
