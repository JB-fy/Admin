...
const table = reactive({
    ...
    {
        dataKey: 'roleName',
        title: t('view.auth.role.name.roleName'),
        key: 'roleName',
        align: 'center',
        width: 150,
    },
    {
        dataKey: 'sceneName',
        title: t('view.auth.role.name.sceneId'),
        key: 'sceneId',
        align: 'center',
        width: 150,
    },
    {
        dataKey: 'tableName',
        title: t('view.auth.role.name.tableId'),
        key: 'tableId',
        align: 'center',
        width: 150,
    },
...
const handleEditCopy = (id: number, type: string = 'edit') => {
    request('/auth/role/info', { id: id }).then((res) => {
...
const handleDelete = (idArr: number[]) => {
    ElMessageBox.confirm('', {
        ...
        request('/auth/role/del', { idArr: idArr }, true).then((res) => {
            getList()
        }).catch(() => { })
    }).catch(() => { })
}
...
const getList = async (resetPage: boolean = false) => {
    ...
    table.loading = true
    try {
        const res = await request('/auth/role/list', param)
