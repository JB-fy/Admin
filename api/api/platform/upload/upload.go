...
type UploadUpdateReq struct {
	...
	UploadType   *uint         `json:"upload_type,omitempty" filter:"-" v:"in:0,1" dc:"类型：0本地 1阿里云OSS"`
	UploadConfig *UploadConfig `json:"upload_config,omitempty" filter:"-" v:"json" dc:"配置。根据upload_type类型设置"`
	Remark       *string       `json:"remark,omitempty" filter:"-" v:"max-length:120" dc:"备注"`
	IsDefault    *uint         `json:"is_default,omitempty" filter:"-" v:"in:0,1" dc:"默认：0否 1是"`
}
