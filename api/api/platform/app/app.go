...
type AppCreateReq struct {
	...
	Remark      *string `json:"remark,omitempty" v:"max-length:255" dc:"备注"`
	IsForcePrev *uint   `json:"is_force_prev,omitempty" v:"in:0,1" dc:"强制更新：0否 1是。注意：只根据前一个版本来设置，与更早之前的版本无关"`
	IsStop      *uint   `json:"is_stop,omitempty" v:"in:0,1" dc:"停用：0否 1是"`
}
...
type AppUpdateReq struct {
	...
	AppType     *uint   `json:"app_type,omitempty" v:"in:0,1" dc:"类型：0安卓 1苹果"`
	PackageName *string `json:"package_name,omitempty" v:"max-length:60" dc:"包名"`
	PackageFile *string `json:"package_file,omitempty" v:"max-length:200|url" dc:"安装包"`
