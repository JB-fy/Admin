...
type LoginSaltReq struct {
	...
	LoginName string `json:"login_name" v:"required|max-length:20" dc:"账号/手机"`
}
...
type LoginLoginReq struct {
	...
	LoginName string `json:"login_name" v:"required|max-length:20" dc:"账号/手机"`
	Password  string `json:"password" v:"required-without:SmsCode|size:32" dc:"密码。加密后发送，公式：md5(md5(md5(密码)+静态密码盐)+动态密码盐)"`
	SmsCode   string `json:"sms_code" v:"required-without:Password|size:4" dc:"短信验证码"`
}
