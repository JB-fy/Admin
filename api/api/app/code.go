...
type CodeSendReq struct {
	...
	Phone  string `json:"phone"  v:"required-if:Scene,0,Scene,1,Scene,2,Scene,4|phone" dc:"手机。scene=0,1,2,4时必须"`
	Email  string `json:"email"  v:"required-if:Scene,10,Scene,11,Scene,12,Scene,14|email" dc:"邮箱。scene=10,11,12,14时必须"`
}
