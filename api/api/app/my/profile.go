...
type ProfileUpdateReq struct {
	...
	SmsCodeToPassword *string     `json:"smsCodeToPassword,omitempty" v:"required-with:Password|size:4" dc:"短信验证码（修改密码用）。修改密码时checkPassword和smsCodeToPassword必传其一"`
	SmsCodeToPhone    *string     `json:"smsCodeToPhone,omitempty" v:"required-with:Phone|size:4" dc:"短信验证码（绑定手机用）。修改密码时checkPassword和smsCodeToPassword必传其一"`
}
