...
type CommonListFilterReq struct {
	...
	IdArr     []uint `c:"idArr,omitempty" p:"idArr" v:"foreach|integer|foreach|min:1"`
	ExcId     *uint  `c:"excId,omitempty" p:"excId" v:"integer|min:1"`
	ExcIdArr  []uint `c:"excIdArr,omitempty" p:"excIdArr" v:"foreach|integer|foreach|min:1"`
...
type CommonUpdateDeleteIdArrReq struct {
	IdArr []uint `c:"idArr,omitempty" p:"idArr" v:"required|array|foreach|integer|foreach|min:1"`
}
