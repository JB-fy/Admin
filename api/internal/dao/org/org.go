...
// 获取daoModel
func (daoThis *orgDao) CtxDaoModel(ctx context.Context, dbOpt ...any) *daoIndex.DaoModel {
	return daoIndex.NewDaoModel(ctx, daoThis, dbOpt...)
}

// 解析分库
func (daoThis *orgDao) ParseDbGroup(ctx context.Context, dbGroupOpt ...any) string {
	group := daoThis.Group()
	// 分库逻辑
	/* if len(dbGroupOpt) > 0 {
	} */
...
// 解析分表
func (daoThis *orgDao) ParseDbTable(ctx context.Context, dbTableOpt ...any) string {
	table := daoThis.Table()
	// 分表逻辑
	/* if len(dbTableOpt) > 0 {
	} */
...
func (daoThis *orgDao) HookDelete(daoModel *daoIndex.DaoModel) gdb.HookHandler {
	return gdb.HookHandler{
		Delete: func(ctx context.Context, in *gdb.HookDeleteInput) (result sql.Result, err error) { //有软删除字段时需改成Update事件
			...
			daoAuth.Role.CtxDaoModel(ctx).Filter(daoAuth.Role.Columns().SceneId, consts.SCENE_ID_ORG).Filter(daoAuth.Role.Columns().RelId, daoModel.IdArr).SetIdArr().HookDelete().Delete()
