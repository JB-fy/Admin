...
var (
	// Upload is a globally accessible object for table upload operations.
	Upload = uploadDao{internal.NewUploadDao()}
)
	...
	return func(m *gdb.Model) *gdb.Model {
		for _, v := range order {
			...
			case daoThis.Columns().UpdatedAt:
				...
				m = m.OrderDesc(daoModel.DbTable + `.` + daoThis.Columns().UploadId)
