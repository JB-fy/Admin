...
var (
	// Config is a globally accessible object for table config operations.
...
func (daoThis *configDao) ParseId(daoModel *daoIndex.DaoModel) string {
	return fmt.Sprintf(`CONCAT_WS( '|', COALESCE( %s, '' ), COALESCE( %s, '' ), COALESCE( %s, '' ) )`, daoModel.DbTable+`.`+daoThis.Columns().SceneId, daoModel.DbTable+`.`+daoThis.Columns().RelId, daoModel.DbTable+`.`+daoThis.Columns().ConfigKey)
}

// 解析Label（未使用代码自动生成，且id字段不在第2个位置时，需手动修改）
func (daoThis *configDao) ParseLabel(daoModel *daoIndex.DaoModel) string {
	return daoModel.DbTable + `.` + daoThis.Columns().RelId
}
	...
	return func(m *gdb.Model) *gdb.Model {
		for k, v := range filter {
			...
			case `id`, `id_arr`:
				...
				m = m.Where(fmt.Sprintf(`(%s, %s, %s) %s (%s)`, daoModel.DbTable+`.`+daoThis.Columns().SceneId, daoModel.DbTable+`.`+daoThis.Columns().RelId, daoModel.DbTable+`.`+daoThis.Columns().ConfigKey, operator, gstr.Join(inStrArr, `, `)))
			case `exc_id`, `exc_id_arr`:
				...
				m = m.Where(fmt.Sprintf(`(%s, %s, %s) %s (%s)`, daoModel.DbTable+`.`+daoThis.Columns().SceneId, daoModel.DbTable+`.`+daoThis.Columns().RelId, daoModel.DbTable+`.`+daoThis.Columns().ConfigKey, operator, gstr.Join(inStrArr, `, `)))
			case `label`:
				m = m.WhereLike(daoModel.DbTable+`.`+daoThis.Columns().RelId, `%`+gconv.String(v)+`%`)
...
func (daoThis *configDao) ParseGroup(group []string, daoModel *daoIndex.DaoModel) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		for _, v := range group {
			switch v {
			case `id`:
				m = m.Group(daoModel.DbTable+`.`+daoThis.Columns().SceneId, daoModel.DbTable+`.`+daoThis.Columns().RelId, daoModel.DbTable+`.`+daoThis.Columns().ConfigKey)
			default:
				if daoThis.Contains(v) {
	...
	return func(m *gdb.Model) *gdb.Model {
		for _, v := range order {
			...
			case `id`:
				...
				m = m.Order(daoModel.DbTable+`.`+daoThis.Columns().SceneId+suffix, daoModel.DbTable+`.`+daoThis.Columns().RelId+suffix, daoModel.DbTable+`.`+daoThis.Columns().ConfigKey+suffix)
			...
			case daoThis.Columns().UpdatedAt:
				...
				m = m.OrderDesc(daoModel.DbTable + `.` + daoThis.Columns().SceneId)
				m = m.OrderDesc(daoModel.DbTable + `.` + daoThis.Columns().RelId)
...
func (daoThis *configDao) GetPluck(ctx context.Context, sceneId consts.SceneId, relId uint, configKeyArr ...string) (config gdb.Record, err error) {
	...
	for k, v := range configTmp {
		config[strings.Replace(k, fmt.Sprintf(`%s|%d|`, sceneId, relId), ``, 1)] = v
	}
	return
}
...
func (daoThis *configDao) Save(ctx context.Context, sceneId consts.SceneId, relId uint, config map[string]any) (err error) {
	...
	for k, v := range config {
		idArr = append(idArr, fmt.Sprintf(`%s|%d|%s`, sceneId, relId, k))
		saveList = append(saveList, map[string]any{
			daoThis.Columns().SceneId:     sceneId,
			daoThis.Columns().RelId:       relId,
			daoThis.Columns().ConfigKey:   k,
			daoThis.Columns().ConfigValue: v,
		})
	}
	_, err = daoThis.CtxDaoModel(ctx).Data(saveList).OnConflict(daoThis.Columns().SceneId, daoThis.Columns().RelId, daoThis.Columns().ConfigKey).Save()
	if err != nil {
		return
	}
	cache.DbData.DelById(ctx, daoThis.CtxDaoModel(ctx), idArr...)
	return
}
