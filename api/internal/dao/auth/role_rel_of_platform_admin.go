...
// 获取daoModel
func (daoThis *roleRelOfPlatformAdminDao) CtxDaoModel(ctx context.Context, dbOpt ...any) *daoIndex.DaoModel {
	return daoIndex.NewDaoModel(ctx, daoThis, dbOpt...)
}

// 解析分库
func (daoThis *roleRelOfPlatformAdminDao) ParseDbGroup(ctx context.Context, dbGroupOpt ...any) string {
	group := daoThis.Group()
	// 分库逻辑
	/* if len(dbGroupOpt) > 0 {
	} */
...
// 解析分表
func (daoThis *roleRelOfPlatformAdminDao) ParseDbTable(ctx context.Context, dbTableOpt ...any) string {
	table := daoThis.Table()
	// 分表逻辑
	/* if len(dbTableOpt) > 0 {
	} */
...
func (daoThis *roleRelOfPlatformAdminDao) ParseId(daoModel *daoIndex.DaoModel) string {
	return daoModel.DbTable + `.` + reflect.ValueOf(daoThis.Columns()).Field(0).String()
}

// 解析Label（未使用代码自动生成，且id字段不在第2个位置时，需手动修改）
func (daoThis *roleRelOfPlatformAdminDao) ParseLabel(daoModel *daoIndex.DaoModel) string {
...
// 处理afterField
func (daoThis *roleRelOfPlatformAdminDao) HandleAfterField(ctx context.Context, record gdb.Record, daoModel *daoIndex.DaoModel) {
