	...
	return gdb.HookHandler{
		Delete: func(ctx context.Context, in *gdb.HookDeleteInput) (result sql.Result, err error) { //有软删除字段时需改成Update事件
			...
			*/
			...
			// daoAdmin.Admin.CtxDaoModel(ctx).Filter(daoAdmin.Admin.Columns().SceneId, daoModel.IdArr). /* SetIdArr(). */ HookDelete().Delete()
			...
			// daoConfig.Config.CtxDaoModel(ctx).Filter(daoConfig.Config.Columns().SceneId, daoModel.IdArr). /* SetIdArr(). */ HookDelete().Delete()
			return
		},
	}
}
...
func (daoThis *sceneDao) CacheGetInfo(ctx context.Context, id string) (info gdb.Record, err error) {
	...
	// 数据修改无需立即同步缓存的表：第一次调用会缓存到当前服务器内存中，所有服务器使用定时器删除缓存，不同服务器定时器执行存在时间差，可能出现不一致的情况（但时间差很小，一般可以接受）
