...
// 获取daoModel
func (daoThis *actionRelToSceneDao) CtxDaoModel(ctx context.Context, dbOpt ...map[string]interface{}) *daoIndex.DaoModel {
	...
	return func(m *gdb.Model) *gdb.Model {
		for k, v := range filter {
			...
			case `id`, `idArr`:
				...
				m = m.Where(`(` + daoModel.DbTable + `.` + daoThis.Columns().ActionId + `, ` + daoModel.DbTable + `.` + daoThis.Columns().SceneId + `) IN (` + gstr.Join(inStrArr, `, `) + `)`)
			case `excId`, `excIdArr`:
				idArr := []string{gconv.String(v)}
				if gvar.New(v).IsSlice() {
					idArr = gconv.SliceStr(v)
				...
				m = m.Where(`(` + daoModel.DbTable + `.` + daoThis.Columns().ActionId + `, ` + daoModel.DbTable + `.` + daoThis.Columns().SceneId + `) NOT IN (` + gstr.Join(inStrArr, `, `) + `)`)
			case `timeRangeStart`:
...
func (daoThis *actionRelToSceneDao) ParseField(field []string, fieldWithParam map[string]interface{}, daoModel *daoIndex.DaoModel) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		for _, v := range field {
			...
			case `id`:
				m = m.Fields(`CONCAT_WS('|', IFNULL(` + daoModel.DbTable + `.` + daoThis.Columns().ActionId + `, ''), IFNULL(` + daoModel.DbTable + `.` + daoThis.Columns().SceneId + `, ''))` + ` AS ` + v)
	...
	return func(m *gdb.Model) *gdb.Model {
		for _, v := range order {
			...
			case `id`:
				...
				m = m.Order(daoModel.DbTable + `.` + daoThis.Columns().SceneId + suffix)
