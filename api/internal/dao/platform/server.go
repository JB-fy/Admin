...
var (
	// Server is a globally accessible object for table platform_server operations.
	Server = serverDao{internal.NewServerDao()}
)

// 获取daoModel
func (daoThis *serverDao) CtxDaoModel(ctx context.Context, dbOpt ...any) *daoIndex.DaoModel {
	return daoIndex.NewDaoModel(ctx, daoThis, dbOpt...)
}

// 解析分库
func (daoThis *serverDao) ParseDbGroup(ctx context.Context, dbGroupOpt ...any) string {
	group := daoThis.Group()
	// 分库逻辑
	/* if len(dbGroupOpt) > 0 {
	} */
...
// 解析分表
func (daoThis *serverDao) ParseDbTable(ctx context.Context, dbTableOpt ...any) string {
	table := daoThis.Table()
	// 分表逻辑
	/* if len(dbTableOpt) > 0 {
	} */
	...
	return func(m *gdb.Model) *gdb.Model {
		for _, v := range order {
			...
			case daoThis.Columns().UpdatedAt:
				...
				m = m.OrderDesc(daoModel.DbTable + `.` + daoThis.Columns().ServerId)
