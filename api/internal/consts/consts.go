...
const (
	...
	CACHE_LOCAL_INTERVAL_MINUTE = 15 * time.Minute //清空本地缓存间隔时间
	...
	CACHE_SALT            = `salt:%s:%s_%d`       //密码盐缓存key。参数：场景ID，手机/邮箱/账号/邮箱，类型
	CACHE_CODE            = `code:%s:%s_%d_%d`    //验证码缓存key。参数：场景ID，手机/邮箱，类型，场景
