...
type ImgOption struct {
	...
	Quality         int            `json:"quality"`           //JPEG压缩数值。范围1~100，大于0生效
	Sigma           float64        `json:"sigma"`             //非JPEG压缩数值。高斯模糊，大于0生效
...
func ImgHandle(imgBytesOfRaw []byte, imgOption ImgOption) (imgBytes []byte, err error) {
	...
	} else if (imgOption.Width == 0 || imgOption.Height == 0) && imgOption.MaxSize == 0 && ((format == imaging.JPEG && imgOption.Quality == 0) || (format != imaging.JPEG && imgOption.Sigma == 0)) {
		return
	}
	...
	quality := ImgQualityInit
	sigma := ImgSigmaInit
	var opts []imaging.EncodeOption
	switch format {
	case imaging.JPEG:
		if imgOption.Quality > 0 {
			quality = imgOption.Quality
	...
	// case imaging.BMP:
	default: //统一使用：高斯模糊
		if imgOption.Sigma > 0 {
			sigma = imgOption.Sigma
