...
func MyGenOptionHandle(ctx context.Context, parser *gcmd.Parser) (option *MyGenOption) {
	...
	for {
		if option.SceneCode != `` {
			count, _ := daoAuth.Scene.DaoModelCtx(ctx).Filter(daoAuth.Scene.Columns().SceneCode, option.SceneCode).Count()
			if count > 0 {
				break
			}
		}
...
func MyGenTplHandle(ctx context.Context, option *MyGenOption) (tpl *MyGenTpl) {
	...
	sceneInfo, _ := daoAuth.Scene.DaoModelCtx(ctx).Filter(daoAuth.Scene.Columns().SceneCode, option.SceneCode).One()
...
func (logicThis *s` + tpl.LogicStructName + `) Create(ctx context.Context, data map[string]interface{}) (id int64, err error) {
	...
	if okPid {
		...
		if pid > 0 {
			pInfo, _ := daoThis.DaoModelCtx(ctx).Filter(daoThis.PrimaryKey(), pid).One()
			if pInfo.IsEmpty() {
...
`
	...
	if okPid {`
		...
		if pid > 0 {
			...
			oldList, _ := daoThis.DaoModelCtx(ctx).Filter(daoThis.PrimaryKey(), daoModelThis.IdArr).All()
			for _, oldInfo := range oldList {
...
`
	if tpl.PidHandle.PidField != `` {
		tplLogic += `
	count, _ := daoThis.DaoModelCtx(ctx).Filter(daoThis.Columns().` + gstr.CaseCamel(tpl.PidHandle.PidField) + `, daoModelThis.IdArr).Count()
	if count > 0 {
...
`
		...
		tplController += `
	daoModelThis := dao` + tpl.ModuleDirCaseCamel + `.` + tpl.TableNameCaseCamel + `.DaoModelCtx(ctx).Filters(filter)`
		if option.IsCount {
			tplController += `
...
`
		...
		tplController += `
	info, err := dao` + tpl.ModuleDirCaseCamel + `.` + tpl.TableNameCaseCamel + `.DaoModelCtx(ctx).Filters(filter).Fields(field).HookSelect().InfoPri()
	if err != nil {
		return
	}
	if info.IsEmpty() {
...
`
	...
	list, err :=dao` + tpl.ModuleDirCaseCamel + `.` + tpl.TableNameCaseCamel + `.DaoModelCtx(ctx).Filters(filter).Fields(field).HookSelect().ListPri()
	if err != nil {
		return
	}
...
func MyGenAction(ctx context.Context, sceneId uint, actionCode string, actionName string) {
	...
	idVar, _ := daoAuth.Action.DaoModelCtx(ctx).Filter(daoAuth.Action.Columns().ActionCode, actionCode).Value(daoAuth.Action.PrimaryKey())
	id := idVar.Int64()
	if id == 0 {
		id, _ = daoAuth.Action.DaoModelCtx(ctx).HookInsert(map[string]interface{}{
			daoAuth.Action.Columns().ActionCode: actionCode,
			daoAuth.Action.Columns().ActionName: actionName,
		}).InsertAndGetId()
	} else {
		daoAuth.Action.DaoModelCtx(ctx).Filter(daoAuth.Action.PrimaryKey(), id).HookUpdate(g.Map{daoAuth.Action.Columns().ActionName: actionName}).Update()
	}
	daoAuth.ActionRelToScene.DaoModelCtx(ctx).Data(map[string]interface{}{
...
func MyGenMenu(ctx context.Context, sceneId uint, menuUrl string, menuName string, menuNameOfEn string) {
	...
	for _, v := range menuNameArr[:len(menuNameArr)-1] {
		pidVar, _ := daoAuth.Menu.DaoModelCtx(ctx).Filters(g.Map{
			daoAuth.Menu.Columns().SceneId:  sceneId,
			daoAuth.Menu.Columns().MenuName: v,
		}).Value(daoAuth.Menu.PrimaryKey())
		if pidVar.Uint() == 0 {
			pid, _ = daoAuth.Menu.DaoModelCtx(ctx).HookInsert(g.Map{
				daoAuth.Menu.Columns().SceneId:   sceneId,
	...
	idVar, _ := daoAuth.Menu.DaoModelCtx(ctx).Filters(g.Map{
		daoAuth.Menu.Columns().SceneId: sceneId,
		daoAuth.Menu.Columns().MenuUrl: menuUrl,
	}).Value(daoAuth.Menu.PrimaryKey())
	id := idVar.Uint()
	if id == 0 {
		daoAuth.Menu.DaoModelCtx(ctx).HookInsert(g.Map{
			daoAuth.Menu.Columns().SceneId:   sceneId,
			daoAuth.Menu.Columns().Pid:       pid,
		...
		daoAuth.Menu.DaoModelCtx(ctx).Filter(daoAuth.Menu.PrimaryKey(), id).
			SetIdArr().
			HookUpdate(g.Map{
				daoAuth.Menu.Columns().MenuName:  menuName,
				daoAuth.Menu.Columns().Pid:       pid,
