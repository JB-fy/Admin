...
func MyGenTplDao(ctx context.Context, option *MyGenOption, tpl *MyGenTpl) {
	...
	for _, column := range tpl.TableColumnList {
		...
		if garray.NewStrArrayFrom([]string{`DeletedAt`, `DeleteAt`, `DeletedTime`, `DeleteTime`}).Contains(fieldCaseCamel) {
			...
			daoParseFilterTmp := `
			case ` + "`timeRangeStart`" + `:
				m = m.WhereGTE(tableThis+` + "`.`" + `+daoThis.Columns().` + fieldCaseCamel + `, v)
			case ` + "`timeRangeEnd`" + `:
				m = m.WhereLTE(tableThis+` + "`.`" + `+daoThis.Columns().` + fieldCaseCamel + `, v)`
			if gstr.Pos(tplDao, daoParseFilterTmp) == -1 {
				daoParseFilter += daoParseFilterTmp
			}
		} else if column[`Key`].String() == `PRI` && column[`Extra`].String() == `auto_increment` { //主键
			...
			case ` + "`tree`" + `:
				...
				m = m.Fields(tableThis + ` + "`.`" + ` + daoThis.Columns().` + fieldCaseCamel + `)
				...
				daoParseOrderTmp := `
			case ` + "`tree`" + `:
				m = m.OrderAsc(tableThis + ` + "`.`" + ` + daoThis.Columns().` + fieldCaseCamel + `)`
				if tpl.PidHandle.SortField != `` {
					daoParseOrderTmp += `
				m = m.OrderAsc(tableThis + ` + "`.`" + ` + daoThis.Columns().` + gstr.CaseCamel(tpl.PidHandle.SortField) + `)`
				}
				daoParseOrderTmp += `
				...
				daoParseJoinTmp := `
		case ` + "`p_`" + ` + tableThis:
			m = m.LeftJoin(tableThis+` + "` AS `" + `+joinCode, joinCode+` + "`.`" + `+daoThis.PrimaryKey()+` + "` = `" + `+tableThis+` + "`.`" + `+daoThis.Columns().` + fieldCaseCamel + `)`
			...
			case daoThis.Columns().` + gstr.CaseCamel(tpl.PidHandle.PidField) + `:
				...
				if gconv.Uint(v) > 0 {
					...
					hookData[` + "`pLevel`" + `] = pInfo[daoThis.Columns().` + gstr.CaseCamel(tpl.PidHandle.LevelField) + `].Uint()
				} else {
					hookData[` + "`pIdPath`" + `] = ` + "`0`" + `
					hookData[` + "`pLevel`" + `] = 0
				}`
			...
			case daoThis.Columns().` + gstr.CaseCamel(tpl.PidHandle.PidField) + `:
				...
				var pLevel uint = 0
				if gconv.Uint(v) > 0 {
					pInfo, _ := daoThis.ParseDbCtx(ctx).Where(daoThis.PrimaryKey(), v).Fields(daoThis.Columns().` + gstr.CaseCamel(tpl.PidHandle.IdPathField) + `, daoThis.Columns().` + gstr.CaseCamel(tpl.PidHandle.LevelField) + `).One()
				...
				updateData[tableThis+` + "`.`" + `+daoThis.Columns().` + gstr.CaseCamel(tpl.PidHandle.IdPathField) + `] = gdb.Raw(` + "`CONCAT('`" + ` + pIdPath + ` + "`-', `" + ` + daoThis.PrimaryKey() + ` + "`)`" + `)
				updateData[tableThis+` + "`.`" + `+daoThis.Columns().` + gstr.CaseCamel(tpl.PidHandle.LevelField) + `] = pLevel + 1`
					if gstr.Pos(tplDao, daoParseUpdateTmp) == -1 {
						daoParseUpdate += daoParseUpdateTmp
					}
...
func (daoThis *` + tpl.TableNameCaseCamelLower + `Dao) UpdateChildIdPathAndLevel(ctx context.Context, newIdPath string, oldIdPath string, newLevel uint, oldLevel uint) {
	...
	daoThis.ParseDbCtx(ctx).WhereLike(daoThis.Columns().` + gstr.CaseCamel(tpl.PidHandle.IdPathField) + `, oldIdPath+` + "`-%`" + `).Data(data).Update()
			...
			case daoThis.Columns().` + fieldCaseCamel + `:
				...
				m = m.OrderDesc(tableThis + ` + "`.`" + ` + daoThis.PrimaryKey())` //追加主键倒序。mysql排序字段有重复值时，分页会导致同一条数据可能在不同页都出现
				if gstr.Pos(tplDao, daoParseOrderTmp) == -1 {
					daoParseOrder += daoParseOrderTmp
				}
			} else if gstr.SubStr(fieldCaseCamelOfRemove, -4) == `Sort` || gstr.SubStr(fieldCaseCamelOfRemove, -6) == `Weight` { //sort,weight等后缀
	...
	dao` + relTable.RelDaoDirCaseCamel + ` "api/internal/dao/` + relTable.RelDaoDir + `"`
		...
		case ` + daoPath + `.ParseDbTable(ctx) + ` + "`" + relTable.RelSuffixCaseSnake + "`" + `:
			...
			if gstr.SubStr(fieldCaseSnake, 0, 6) == `start_` { //start_前缀
				daoParseFilterTmp := `
			case daoThis.Columns().` + fieldCaseCamel + `:
				m = m.WhereLTE(tableThis+` + "`.`" + `+k, v)`
				if column[`Null`].String() == `NO` && column[`Default`].String() == `` {
					daoParseFilterTmp = `
			case daoThis.Columns().` + fieldCaseCamel + `:
				m = m.Where(m.Builder().WhereLTE(tableThis+` + "`.`" + `+k, v).WhereOrNull(tableThis + ` + "`.`" + ` + k))`
				}
				if gstr.Pos(tplDao, daoParseFilterTmp) == -1 {
					daoParseFilter += daoParseFilterTmp
				}
				...
				daoParseFilterTmp := `
			case daoThis.Columns().` + fieldCaseCamel + `:
				m = m.WhereGTE(tableThis+` + "`.`" + `+k, v)`
				if column[`Null`].String() == `NO` && column[`Default`].String() == `` {
					daoParseFilterTmp = `
			case daoThis.Columns().` + fieldCaseCamel + `:
				m = m.Where(m.Builder().WhereGTE(tableThis+` + "`.`" + `+k, v).WhereOrNull(tableThis + ` + "`.`" + ` + k))`
				}
				if gstr.Pos(tplDao, daoParseFilterTmp) == -1 {
					daoParseFilter += daoParseFilterTmp
				}
...
func MyGenTplApi(ctx context.Context, option *MyGenOption, tpl *MyGenTpl) {
	...
	for _, column := range tpl.TableColumnList {
		...
		if garray.NewStrArrayFrom([]string{`DeletedAt`, `DeleteAt`, `DeletedTime`, `DeleteTime`}).Contains(fieldCaseCamel) {
			...
			typeRes = `*string`
		} else if (gstr.SubStr(fieldCaseCamelOfRemove, -6) == `Status` || gstr.SubStr(fieldCaseCamelOfRemove, -4) == `Type` || gstr.SubStr(fieldCaseCamelOfRemove, -6) == `Method` || gstr.SubStr(fieldCaseCamelOfRemove, -6) == `Gender`) && ((gstr.Pos(column[`Type`].String(), `int`) != -1 && gstr.Pos(column[`Type`].String(), `point`) == -1) || gstr.Pos(column[`Type`].String(), `char`) != -1) { //status,type,method,gender等后缀
			...
			if gstr.SubStr(fieldCaseSnake, 0, 6) == `start_` || gstr.SubStr(fieldCaseSnake, 0, 4) == `end_` { //start_前缀 //end_前缀
				typeReqFilter = `*gtime.Time`
			}
		} else if gstr.Pos(column[`Type`].String(), `json`) != -1 { //json类型
			if column[`Null`].String() == `NO` {
...
import md5 from 'js-md5'`
        ...
        ` + field + `: [
            { type: 'integer', min: 0, max: 100, trigger: 'change', message: t('validation.between.number', { min: 0, max: 100 }) },
				...
				viewSaveField += `
                <el-form-item :label="t('` + tpl.ModuleDirCaseCamelLowerReplace + `.` + tpl.TableNameCaseCamelLower + `.name.` + field + `')" prop="` + field + `">
                    <el-input-number v-model="saveForm.data.` + field + `" :precision="0" :min="0" :max="100" :step="1" :step-strictly="true" controls-position="right" :value-on-clear="` + column[`Default`].String() + `" />
                    <label>
        ...
        ` + field + `: [
            { type: 'integer', min: 0, trigger: 'change', message: t('validation.min.number', { min: 0 }) },
					...
					viewSaveField += `
                <el-form-item :label="t('` + tpl.ModuleDirCaseCamelLowerReplace + `.` + tpl.TableNameCaseCamelLower + `.name.` + field + `')" prop="` + field + `">
                    <el-input-number v-model="saveForm.data.` + field + `" :placeholder="t('` + tpl.ModuleDirCaseCamelLowerReplace + `.` + tpl.TableNameCaseCamelLower + `.name.` + field + `')" :min="0" :controls="false"/>
                </el-form-item>`
				} else {
					viewSaveRule += `
        ` + field + `: [
            { type: 'integer', trigger: 'change', message: t('validation.input') },
					...
					viewSaveField += `
                <el-form-item :label="t('` + tpl.ModuleDirCaseCamelLowerReplace + `.` + tpl.TableNameCaseCamelLower + `.name.` + field + `')" prop="` + field + `">
                    <el-input-number v-model="saveForm.data.` + field + `" :placeholder="t('` + tpl.ModuleDirCaseCamelLowerReplace + `.` + tpl.TableNameCaseCamelLower + `.name.` + field + `')" :controls="false"/>
                </el-form-item>`
				}
			}
		} else if gstr.Pos(column[`Type`].String(), `decimal`) != -1 || gstr.Pos(column[`Type`].String(), `double`) != -1 || gstr.Pos(column[`Type`].String(), `float`) != -1 { //float类型
        ...
        ` + field + `: [
            { type: 'number'/* 'float' */, min: 0, trigger: 'change', message: t('validation.min.number', { min: 0 }) },    // 类型float值为0时验证不能通过
				...
				viewSaveField += `
                <el-form-item :label="t('` + tpl.ModuleDirCaseCamelLowerReplace + `.` + tpl.TableNameCaseCamelLower + `.name.` + field + `')" prop="` + field + `">
                    <el-input-number v-model="saveForm.data.` + field + `" :placeholder="t('` + tpl.ModuleDirCaseCamelLowerReplace + `.` + tpl.TableNameCaseCamelLower + `.name.` + field + `')" :min="0" :precision="` + resultFloat[2] + `" :controls="false"/>
                </el-form-item>`
			} else {
				viewSaveRule += `
        ` + field + `: [
            { type: 'number'/* 'float' */, trigger: 'change', message: t('validation.input') },    // 类型float值为0时验证不能通过
				...
				viewSaveField += `
                <el-form-item :label="t('` + tpl.ModuleDirCaseCamelLowerReplace + `.` + tpl.TableNameCaseCamelLower + `.name.` + field + `')" prop="` + field + `">
                    <el-input-number v-model="saveForm.data.` + field + `" :placeholder="t('` + tpl.ModuleDirCaseCamelLowerReplace + `.` + tpl.TableNameCaseCamelLower + `.name.` + field + `')" :precision="` + resultFloat[2] + `" :controls="false"/>
                </el-form-item>`
			}
		} else if gstr.Pos(column[`Type`].String(), `timestamp`) != -1 || gstr.Pos(column[`Type`].String(), `date`) != -1 { //timestamp或datetime或date类型
			typeDatePicker := `datetime`
