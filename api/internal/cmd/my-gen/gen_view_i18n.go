...
func getViewI18nExtendMiddleMany(tplEM handleExtendMiddle) (viewI18n myGenViewI18n) {
	// viewI18nTmp := getViewI18nFieldList(tplEM.tpl, tplEM.FieldArr...)
	if len(tplEM.FieldArr) == 1 {
		...
		for _, v := range tpl.FieldList {
			...
			viewI18n.Add(viewI18nField, tplEM.FieldVal)
		}
	} else {
		viewI18n.name = append(viewI18n.name, tplEM.FieldVal+`Name: '列表',`)
		...
		viewI18n.name = append(viewI18n.name, tplEM.FieldVal+`: {`+gstr.Join(append([]string{``}, viewI18nTmp.name...), `
			`)+`
        },`)
		viewI18n.status = append(viewI18n.status, tplEM.FieldVal+`: {`+gstr.Join(append([]string{``}, viewI18nTmp.status...), `
			`)+`
        },`)
		viewI18n.tip = append(viewI18n.tip, tplEM.FieldVal+`: {`+gstr.Join(append([]string{``}, viewI18nTmp.tip...), `
			`)+`
        },`)
	}
	return
