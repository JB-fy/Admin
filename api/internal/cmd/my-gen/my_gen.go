/*
后台常用生成示例：./main myGen -sceneCode=platform -dbGroup=default -dbTable=test -removePrefixCommon= -removePrefixAlone= -commonName=测试管理/测试 -isList=1 -isCount=1 -isInfo=1 -isCreate=1 -isUpdate=1 -isDelete=1 -isApi=1 -isAuthAction=1 -isView=1 -isResetLogic=0
...
表名统一使用蛇形命名。不同功能表按以下表规则命名
	...
	中间表（一对多）：表命名：主表名_rel_to_xxxx 或 xxxx_rel_of_主表名，同模块时，后面部分可省略独有前缀，并存在至少2个与关联表（主键 或 表名去掉前缀 + ID）同名的id后缀字段。所有表的关联字段设为：联合主键或联合唯一索引
