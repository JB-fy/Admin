...
表字段名统一使用蛇形或小驼峰命名（建议：蛇形。mysql两者都支持；postgresql仅支持蛇形；）
	...
	字段按以下规则命名时，会做特殊处理，其它情况根据字段类型做默认处理
		...
		常用命名(字段含[_of_]时，会忽略[_of_]及其之后的部分)：
			...
			图片			命名：icon,cover,avatar,img,img_list,imgList,img_arr,imgArr,image,image_list,imageList,image_arr,imageArr等后缀；	类型：varchar或json或text
			视频			命名：video,video_list,videoList,video_arr,videoArr等后缀；		类型：varchar或json或text
			音频			命名：audio,audio_list,audioList,audio_arr,audioArr等后缀；		类型：varchar或json或text
			文件			命名：file,file_list,fileList,file_arr,fileArr等后缀；			类型：varchar或json或text
			数组			命名：list,arr等后缀；											类型：varchar或json或text；
*/
...
func Run(ctx context.Context, parser *gcmd.Parser) {
	...
	tpl := createTpl(ctx, option, option.DbGroup, option.DbTable, option.RemovePrefixCommon, option.RemovePrefixAlone, true, true)
