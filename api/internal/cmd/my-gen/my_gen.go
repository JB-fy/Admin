...
平台后台生成示例：
	...
	./main myGen -dbGroup=default -dbTable=org -removePrefixCommon= -removePrefixAlone= -isApi=1 -isResetLogic=1 -isAuthAction=1 -commonName=机构管理/机构 -isView=1 -sceneCode=platform -isList=1 -isCount=1 -isInfo=1 -isCreate=1 -isUpdate=1 -isDelete=1
	./main myGen -dbGroup=default -dbTable=org_admin -removePrefixCommon= -removePrefixAlone=org_ -isApi=1 -isResetLogic=1 -isAuthAction=1 -commonName=权限管理/机构管理员 -isView=1 -sceneCode=platform -isList=1 -isCount=1 -isInfo=1 -isCreate=1 -isUpdate=1 -isDelete=1
...
表字段名统一使用蛇形或小驼峰命名（建议：蛇形。mysql两者都支持；postgresql仅支持蛇形；）
	...
	字段按以下规则命名时，会做特殊处理，其它情况根据字段类型做默认处理
		...
		常用命名(字段含[_of_]时，会忽略[_of_]及其之后的部分)：
			...
			状态|类型		命名：status,type,method,pos,position,gender,currency等后缀；	类型：int等类型或varchar或char；	注释：多状态之间用[\s,，;；]等字符分隔。示例（状态：0待处理 1已处理 2驳回 yes是 no否）
...
type myGenOption struct {
	...
	/*
		是否重置logic层。一般情况下不建议重置，原因：logic层生成基本不会有任何变化，且常会在该层手写一些逻辑验证和自定义方法。只建议在以下两种情况下重置：
			...
			2、表新增或删除了对logic层生成代码有影响的字段。以下几种情况下会对logic有影响：
				当前表新增或删除pid字段时，新增更新方法内会生成验证代码（验证pid是否存在于当前表）
				当前表新增或删除id后缀字段时，新增更新方法内会生成验证代码（验证id是否存在于关联表）
				其它表新增或删除与当前表主键关联的id后缀字段时，删除方法内会生成验证代码（验证id是否正被其它表使用，是否可以删除）
	*/
