...
func (c *Action) List(r *ghttp.Request) {
	var param *api.ReqActionList
	err := r.Parse(&param)
	if err != nil {
	...
	case "platformAdmin":
		...
		list, err := service.Action().List(r.Context(), filter, param.Field, [2]string{}, int((param.Page-1)*param.Limit), int(param.Limit))
		if err != nil {
