...
func (controllerThis *Users) Update(ctx context.Context, req *apiUsers.UsersUpdateReq) (res *api.CommonNoDataRes, err error) {
	/**--------参数处理 开始--------**/
	filter := gconv.Map(req.UsersUpdateDeleteFilter, gconv.MapOption{Deep: true, OmitEmpty: true})
	data := gconv.Map(req.UsersUpdateData, gconv.MapOption{Deep: true, OmitEmpty: true})
	if len(data) == 0 {
