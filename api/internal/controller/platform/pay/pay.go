...
func (controllerThis *Pay) Create(ctx context.Context, req *apiPay.PayCreateReq) (res *api.CommonCreateRes, err error) {
	/**--------参数处理 开始--------**/
	data := gconv.Map(req.PayCreateData, gconv.MapOption{Deep: true, OmitEmpty: true})
	/**--------参数处理 结束--------**/
...
func (controllerThis *Pay) Update(ctx context.Context, req *apiPay.PayUpdateReq) (res *api.CommonNoDataRes, err error) {
	/**--------参数处理 开始--------**/
	filter := gconv.Map(req.PayUpdateDeleteFilter, gconv.MapOption{Deep: true, OmitEmpty: true})
	data := gconv.Map(req.PayUpdateData, gconv.MapOption{Deep: true, OmitEmpty: true})
	if len(data) == 0 {
