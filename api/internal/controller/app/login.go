...
func (controllerThis *Login) Login(ctx context.Context, req *apiCurrent.LoginLoginReq) (res *api.CommonTokenRes, err error) {
	...
	if req.Password != `` { //密码
		...
		code, _ := cache.NewCode(ctx, sceneId, phone, 0).Get() //场景：0登录(手机)
		...
		code, _ := cache.NewCode(ctx, sceneId, email, 10).Get() //场景：10登录(邮箱)
...
func (controllerThis *Login) Register(ctx context.Context, req *apiCurrent.LoginRegisterReq) (res *api.CommonTokenRes, err error) {
	...
	sceneId := sceneInfo[daoAuth.Scene.Columns().SceneId].String()
	if req.Phone != `` {
		code, _ := cache.NewCode(ctx, sceneId, req.Phone, 1).Get() //场景：1注册(手机)
		if code == `` || code != req.SmsCode {
			err = utils.NewErrorCode(ctx, 39991999, ``)
			return
		}
	...
	if req.Email != `` {
		code, _ := cache.NewCode(ctx, sceneId, req.Email, 11).Get() //场景：11注册(邮箱)
		if code == `` || code != req.EmailCode {
			err = utils.NewErrorCode(ctx, 39991999, ``)
			return
		}
...
func (controllerThis *Login) PasswordRecovery(ctx context.Context, req *apiCurrent.LoginPasswordRecoveryReq) (res *api.CommonNoDataRes, err error) {
	...
	filter := g.Map{}
	if req.Phone != `` {
		code, _ := cache.NewCode(ctx, sceneId, req.Phone, 2).Get() //场景：2密码找回(手机)
		if code == `` || code != req.SmsCode {
			err = utils.NewErrorCode(ctx, 39991999, ``)
			return
		}
		...
		code, _ := cache.NewCode(ctx, sceneId, req.Email, 12).Get() //场景：12密码找回(邮箱)
		if code == `` || code != req.EmailCode {
			err = utils.NewErrorCode(ctx, 39991999, ``)
			return
		}
...
func (controllerThis *Login) OneClickPreInfo(ctx context.Context, req *apiCurrent.LoginOneClickPreInfoReq) (res *apiCurrent.LoginOneClickPreInfoRes, err error) {
	...
	case `oneClickOfWx`: //微信
		res.CodeUrlOfWx, err = one_click.NewOneClickOfWxHandler(ctx).CodeUrl(req.RedirectUriOfWx, req.ScopeOfWx, req.StateOfWx, req.ForcePopupOfWx)
...
func (controllerThis *Login) OneClick(ctx context.Context, req *apiCurrent.LoginOneClickReq) (res *api.CommonTokenRes, err error) {
	...
	switch req.OneClickType {
	case `oneClickOfWx`: //微信
		accessToken, errTmp := one_click.NewOneClickOfWxHandler(ctx).AccessToken(ctx, req.CodeOfWx)
		if errTmp != nil {
			err = errTmp
			return
		}
		...
		if garray.NewStrArrayFrom([]string{`snsapi_userinfo`, `snsapi_login`}).Contains(accessToken.Scope) {
			userInfo, errTmp := one_click.NewOneClickOfWxHandler(ctx).UserInfo(ctx, accessToken.Openid, accessToken.AccessToken)
			if errTmp != nil {
				err = errTmp
				return
			}
