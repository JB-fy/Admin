...
// 解析insert
func (daoScene *sceneDao) ParseInsert(insert []map[string]interface{}, fill ...bool) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		...
		for index, item := range insert {
			...
			for k, v := range item {
				switch k {
				case "id":
					insertData[index][daoScene.PrimaryKey()] = v
				default:
					//数据库不存在的字段过滤掉，未传值默认true
					if (len(fill) == 0 || fill[0]) && !daoScene.ColumnArrG().Contains(k) {
						continue
					}
...
// 解析update
func (daoScene *sceneDao) ParseUpdate(update map[string]interface{}, fill ...bool) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		updateData := map[string]interface{}{}
		for k, v := range update {
			switch k {
			case "id":
				updateData[daoScene.Table()+"."+daoScene.PrimaryKey()] = v
			default:
				//数据库不存在的字段过滤掉，未传值默认true
				if (len(fill) == 0 || fill[0]) && !daoScene.ColumnArrG().Contains(k) {
					continue
				}
				updateData[daoScene.Table()+"."+k] = v
			}
		}
...
func (daoScene *sceneDao) ParseField(field []string, joinTableArr *[]string) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		...
		for _, v := range field {
			...
			default:
				if daoScene.ColumnArrG().Contains(v) {
					m = m.Fields(daoScene.Table() + "." + v)
				} else {
					m = m.Fields(v)
				}
			}
		...
		if len(afterField) > 0 {
			m = m.Hook(daoScene.AfterField(afterField))
		}
		return m
	}
}
...
func (daoScene *sceneDao) ParseFilter(filter map[string]interface{}, joinTableArr *[]string) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		for k, v := range filter {
			switch k {
			case "id", "idArr":
				m = m.Where(daoScene.Table()+"."+daoScene.PrimaryKey(), v)
			case "excId":
				m = m.WhereNot(daoScene.Table()+"."+daoScene.PrimaryKey(), v)
			case "excIdArr":
				m = m.WhereNotIn(daoScene.Table()+"."+daoScene.PrimaryKey(), v)
			case "startTime":
				m = m.WhereGTE(daoScene.Table()+".createTime", v)
			case "endTime":
				m = m.WhereLTE(daoScene.Table()+".createTime", v)
			case "keyword":
				keywordField := strings.ReplaceAll(daoScene.PrimaryKey(), "Id", "Name")
				switch v := v.(type) {
				case *string:
					m = m.WhereLike(daoScene.Table()+"."+keywordField, *v)
				case string:
					m = m.WhereLike(daoScene.Table()+"."+keywordField, v)
				default:
...
func (daoScene *sceneDao) ParseGroup(group []string, joinTableArr *[]string) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		for _, v := range group {
			switch v {
			case "id":
				m = m.Group(daoScene.Table() + "." + daoScene.PrimaryKey())
			default:
				if daoScene.ColumnArrG().Contains(v) {
					m = m.Group(daoScene.Table() + "." + v)
				} else {
					m = m.Group(v)
				}
			}
...
func (daoScene *sceneDao) ParseOrder(order [][2]string, joinTableArr *[]string) func(m *gdb.Model) *gdb.Model {
	return func(m *gdb.Model) *gdb.Model {
		for _, v := range order {
			switch v[0] {
			case "id":
				m = m.Order(daoScene.Table()+"."+daoScene.PrimaryKey(), v[1])
			default:
				if daoScene.ColumnArrG().Contains(v[0]) {
					m = m.Order(daoScene.Table()+"."+v[0], v[1])
				} else {
					m = m.Order(v[0], v[1])
				}
			}
	...
	return func(m *gdb.Model) *gdb.Model {
		...
		if !garray.NewStrArrayFrom(*joinTableArr).Contains(xxxxTable) {
			*joinTableArr = append(*joinTableArr, xxxxTable)
			m = m.LeftJoin(xxxxTable, xxxxTable+"."+daoScene.PrimaryKey()+" = "+daoScene.Table()+"."+daoScene.PrimaryKey())
		} */
		}
		return m
	}
...
// 获取数据后，再处理的字段
func (daoScene *sceneDao) AfterField(afterField []string) gdb.HookHandler {
