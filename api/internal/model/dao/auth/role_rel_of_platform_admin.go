...
// 解析order
func (daoThis *roleRelOfPlatformAdminDao) ParseOrder(order [][2]string, joinTableArr *[]string) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		for _, v := range order {
			switch v[0] {
			case "id":
...
// 获取数据后，再处理的字段
func (daoThis *roleRelOfPlatformAdminDao) AfterField(afterField []string) gdb.HookHandler {
...
func (daoThis *roleRelOfPlatformAdminDao) CommonDbCtx(ctx context.Context, filter map[string]interface{}, field []string, joinTableArr *[]string, dbSelDataList ...map[string]interface{}) *gdb.Model {
