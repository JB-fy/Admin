...
// 详情
func (logicThis *sRequest) Info(ctx context.Context, filter map[string]interface{}, field ...[]string) (info gdb.Record, err error) {
	daoThis := daoAuth.Request
	joinTableArr := []string{}
	model := daoThis.ParseDbCtx(ctx)
	model = model.Handler(daoThis.ParseFilter(filter, &joinTableArr))
...
// 创建
func (logicThis *sRequest) Create(ctx context.Context, data []map[string]interface{}) (id int64, err error) {
	daoThis := daoAuth.Request
	model := daoThis.ParseDbCtx(ctx)
...
// 更新
func (logicThis *sRequest) Update(ctx context.Context, data map[string]interface{}, filter map[string]interface{}) (row int64, err error) {
	daoThis := daoAuth.Request
	joinTableArr := []string{}
	model := daoThis.ParseDbCtx(ctx)
...
// 删除
func (logicThis *sRequest) Delete(ctx context.Context, filter map[string]interface{}) (row int64, err error) {
	daoThis := daoAuth.Request
	joinTableArr := []string{}
	model := daoThis.ParseDbCtx(ctx)
	model = model.Handler(daoThis.ParseFilter(filter, &joinTableArr))
