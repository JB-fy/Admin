...
var DbDataLocal = dbDataLocal{
	cacheKeyMap: map[string]uint8{ // 可根据 数据库或表 使用不同缓存库。不同缓存库每隔（值*consts.CACHE_LOCAL_INTERVAL_MINUTE）时间会删除缓存，可配合定时器使全部服务器同步缓存
		// `default:auth_scene`: 0, //默认为0，即第一个缓存库（默认凌晨3点清空）
		`default:auth_menu`:   1,
		`default:auth_action`: 1,
		// `default:upload`: 0,
		// `default:pay`: 0,
		// `default:pay_channel`: 0,
		// `default:pay_scene`: 0,
		// `default:app`: 0,
	},
	methodCode:        ``,
...
func (cacheThis *dbDataLocal) del(_ context.Context, daoModel *dao.DaoModel, method string, code any) {
	cacheThis.cache(daoModel).Delete(cacheThis.key(daoModel, method, code))
}

func (cacheThis *dbDataLocal) delById(_ context.Context, daoModel *dao.DaoModel, method string, idArr []any) {
