...
abstract class AbstractController
{
    ...
    public function __construct()
    {
        ...
        if (empty($this->actionCodePrefix)) {
            $this->actionCodePrefix = lcfirst(str_replace('\\', '',  substr($className, strpos($className, $strReplace) + strlen($strReplace))));
        }
    }
        ...
        switch ($funcName) {
            ...
            case 'list':
                if (!empty($data)) {
                    ...
                    if (isset($data['sort'])) {
                        $kArr = explode(' ', $data['sort']);
                        $data['order'] = [$kArr[0] => $kArr[1] ?? 'desc'];
                        unset($data['sort']);
                    }
            ...
            case 'update':
                ...
                if (count($data) < 2) { //更新除了id还必须有其它参数，所以至少需要两个参数
