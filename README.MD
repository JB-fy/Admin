...
如果您将此项目用于商业用途，请遵守[Apache2.0协议](./LICENSE)并保留作者技术支持声明。您需保留如下版权声明信息，其余信息功能不做任何限制。如需剔除请联系作者购买授权（往下翻有作者微信）

<img src="./favicon.ico" width="1000">
...
#### GO服务端（推荐使用，有代码自动生成功能。代码目录：[api](./api)）
```bash
# 服务启动
...
#### PHP服务端（无代码自动生成功能。代码目录：[api_sw](./api_sw)）
```bash
# 服务启动
...
#### VUE前端（代码目录：[dev/platform](./dev/platform)）
```bash
# 服务启动
npm run dev
# 打包
