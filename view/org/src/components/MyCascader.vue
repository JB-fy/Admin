...
const props = defineProps({
    ...
    /**
     * 接口。格式：{ code: string, param: object, transform: function, pidField: string, isPidStr: true }
...
const cascader = reactive({
    ...
    props: {
        ...
        lazyLoad: (node: any, resolve: any) => {
            if (node.level == 0) {
                cascader.api.param.filter[cascader.api.pidField] = props.api.isPidStr ? '' : 0
            } else {
