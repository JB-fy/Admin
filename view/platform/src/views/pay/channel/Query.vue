...
<template>
    <el-form class="query-form" :ref="(el: any) => queryForm.ref = el" :model="queryCommon.data" :inline="true" @keyup.enter="queryForm.submit">
        ...
        <el-form-item prop="channel_name">
            <el-input v-model="queryCommon.data.channel_name" :placeholder="t('pay.channel.name.channel_name')" maxlength="30" :clearable="true" />
        </el-form-item>
        <el-form-item prop="scene_id">
            <my-select v-model="queryCommon.data.scene_id" :placeholder="t('pay.channel.name.scene_id')" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/pay/scene/list' }" />
        </el-form-item>
