...
const table = reactive({
    columns: [
        ...
        },
        {
            ...
            title: t('pay.channel.name.channel_name'),
            key: 'channel_name',
            align: 'center',
            width: 150,
            cellRenderer: (props: any): any => {
                ...
                return [
                    <el-input
                        ...
                        onBlur={() => {
                            props.rowData.editChannelName.isEdit = false
                            if (props.rowData.channel_name == props.rowData.editChannelName.oldValue) {
                                return
                            }
                            if (!props.rowData.channel_name) {
        ...
        {
            dataKey: 'pay_name',
            title: t('pay.channel.name.pay_id'),
            key: 'pay_id',
            align: 'center',
            width: 150,
        },
        {
            ...
            cellRenderer: (props: any): any => {
                ...
                let statusList = tm('pay.channel.status.pay_method') as { value: any; label: string }[]
                let statusIndex = statusList.findIndex((item) => item.value == props.rowData.pay_method)
                return <el-tag type={tagType[statusIndex % tagType.length]}>{statusList[statusIndex]?.label}</el-tag>
            },
        },
        {
            dataKey: 'sort',
            title: t('pay.channel.name.sort'),
            key: 'sort',
            align: 'center',
            width: 100,
            sortable: true,
        ...
        },
        {
            ...
            title: t('pay.channel.name.total_amount'),
            key: 'total_amount',
            align: 'center',
            width: 150,
        },
        {
            dataKey: 'is_stop',
            title: t('pay.channel.name.is_stop'),
            key: 'is_stop',
            align: 'center',
            width: 100,
            cellRenderer: (props: any): any => {
...
const handleEditCopy = (id: number, type: string = 'edit') => {
    request(t('config.VITE_HTTP_API_PREFIX') + '/pay/channel/info', { id: id }).then((res) => {
...
const getList = async (resetPage: boolean = false) => {
    ...
    table.loading = true
    try {
        const res = await request(t('config.VITE_HTTP_API_PREFIX') + '/pay/channel/list', param)
    ...
    <el-row class="main-table-tool">
        ...
        <el-col :span="8" style="text-align: right">
            <el-space :size="10" style="height: 100%; margin-right: 10px">
                <my-export-button i18nPrefix="pay.channel" :headerList="table.columns" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/pay/channel/list', param: { filter: queryCommon.data, sort: table.sort.key + ' ' + table.sort.order } }" />
