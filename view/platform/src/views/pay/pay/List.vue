                ...
                return [
                    <el-input
                        ...
                        onBlur={() => {
                            ...
                            handleUpdate({ id_arr: [props.rowData.id], pay_name: props.rowData.pay_name }).catch(() => (props.rowData.pay_name = props.rowData.editPayName.oldValue))
                        }}
                        onKeydown={(event: any) => {
...
const handleEditCopy = (id: number, type: string = 'edit') => {
    request(t('config.VITE_HTTP_API_PREFIX') + '/pay/pay/info', { id: id })
        .then((res) => {
...
const handleDelete = (idArr: number[]) => {
    ElMessageBox.confirm('', {
        ...
        .then(() => {
            request(t('config.VITE_HTTP_API_PREFIX') + '/pay/pay/del', { id_arr: idArr }, true)
                .then((res) => {
                    getList()
                })
                .catch(() => {})
