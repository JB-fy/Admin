...
const table = reactive({
    columns: [
        ...
        {
            dataKey: 'scene_name',
            title: t('pay.scene.name.scene_name'),
            key: 'scene_name',
            align: 'center',
            width: 150,
            cellRenderer: (props: any): any => {
                if (!authAction.isUpdate) {
                    return [
                        <el-text line-clamp="2" title={props.rowData.scene_name}>
                ...
                if (!props.rowData?.editSceneName?.isEdit) {
                    return [
                        <el-text class="inline-edit" type="primary" line-clamp="2" title={props.rowData.scene_name} onClick={() => (props.rowData.editSceneName = { isEdit: true, oldValue: props.rowData.scene_name })}>
                            {props.rowData.scene_name}
                        </el-text>,
                    ]
                }
                ...
                return [
                    <el-input
                        ...
                        placeholder={t('pay.scene.name.scene_name')}
                        maxlength={30}
                        ...
                        onBlur={() => {
                            ...
                            handleUpdate(props.rowData.id, { scene_name: props.rowData.scene_name }).catch(() => (props.rowData.scene_name = props.rowData.editSceneName.oldValue))
                        }}
        ...
        {
            dataKey: 'is_stop',
            title: t('pay.scene.name.is_stop'),
            key: 'is_stop',
            align: 'center',
            width: 100,
            cellRenderer: (props: any): any => {
...
const handleEditCopy = (id: number, type: string = 'edit') => {
    request(t('config.VITE_HTTP_API_PREFIX') + '/pay/scene/info', { id: id }).then((res) => {
    ...
    ElMessageBox.confirm('', {
        ...
        beforeClose: (action: Action, instance: MessageBoxState, done: Function) => {
            if (action == 'confirm') {
                ...
                request(t('config.VITE_HTTP_API_PREFIX') + '/pay/scene/del', { [Array.isArray(id) ? 'id_arr' : 'id']: id }, true)
...
const handleUpdate = async (id: number | number[], param: { [propName: string]: any }) => {
    ...
    await request(t('config.VITE_HTTP_API_PREFIX') + '/pay/scene/update', param, true)
}
...
const getList = async (resetPage: boolean = false) => {
    ...
    table.loading = true
    try {
        const res = await request(t('config.VITE_HTTP_API_PREFIX') + '/pay/scene/list', param)
    ...
    <el-row class="main-table-tool">
        ...
        <el-col :span="8" style="text-align: right">
            <el-space :size="10" style="height: 100%; margin-right: 10px">
                <my-export-button i18nPrefix="pay.scene" :headerList="table.columns" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/pay/scene/list', param: { filter: queryCommon.data, sort: table.sort.key + ' ' + table.sort.order } }" />
