        ...
        saveForm.ref.validate(async (valid: boolean) => {
            ...
            try {
                if (param?.id) {
                    ...
                    await request(t('config.VITE_HTTP_API_PREFIX') + '/pay/scene/create', param, true)
                }
    ...
    <el-drawer class="save-drawer" :ref="(el: any) => saveDrawer.ref = el" v-model="saveCommon.visible" :title="saveCommon.title" :size="saveDrawer.size" :before-close="saveDrawer.beforeClose">
        <el-scrollbar>
            <el-form :ref="(el: any) => saveForm.ref = el" :model="saveForm.data" :rules="saveForm.rules" label-width="auto" :status-icon="true" :scroll-to-error="true">
                <el-form-item :label="t('pay.scene.name.scene_name')" prop="scene_name">
                    <el-input v-model="saveForm.data.scene_name" :placeholder="t('pay.scene.name.scene_name')" maxlength="30" :show-word-limit="true" :clearable="true" />
                </el-form-item>
                <el-form-item :label="t('pay.scene.name.remark')" prop="remark">
                    <el-input v-model="saveForm.data.remark" type="textarea" :autosize="{ minRows: 3 }" maxlength="120" :show-word-limit="true" />
                </el-form-item>
                <el-form-item :label="t('pay.scene.name.is_stop')" prop="is_stop">
                    <el-switch
