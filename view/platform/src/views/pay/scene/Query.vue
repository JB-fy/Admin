...
<template>
    <el-form class="query-form" :ref="(el: any) => queryForm.ref = el" :model="queryCommon.data" :inline="true" @keyup.enter="queryForm.submit">
        ...
        <el-form-item prop="scene_name">
            <el-input v-model="queryCommon.data.scene_name" :placeholder="t('pay.scene.name.scene_name')" maxlength="30" :clearable="true" />
        </el-form-item>
        <el-form-item prop="is_stop">
            <el-select-v2 v-model="queryCommon.data.is_stop" :options="tm('common.status.whether')" :placeholder="t('pay.scene.name.is_stop')" :clearable="true" style="width: 86px" />
        </el-form-item>
