...
const table = reactive({
    columns: [
        ...
        {
            dataKey: 'sceneName',
            title: t('auth.scene.name.sceneName'),
            key: 'sceneName',
            align: 'center',
            width: 150,
        },
        {
            dataKey: 'sceneCode',
            title: t('auth.scene.name.sceneCode'),
            key: 'sceneCode',
            align: 'center',
            width: 150,
        },
        {
            dataKey: 'sceneConfig',
            title: t('auth.scene.name.sceneConfig'),
            key: 'sceneConfig',
            align: 'center',
            width: 200,
            hidden: true,
        ...
        {
            dataKey: 'isStop',
            title: t('auth.scene.name.isStop'),
            key: 'isStop',
            align: 'center',
            width: 100,
            cellRenderer: (props: any): any => {
...
const handleEditCopy = (id: number, type: string = 'edit') => {
    request(t('config.VITE_HTTP_API_PREFIX') + '/auth/scene/info', { id: id })
        .then((res) => {
...
const handleDelete = (idArr: number[]) => {
    ElMessageBox.confirm('', {
        ...
        .then(() => {
            request(t('config.VITE_HTTP_API_PREFIX') + '/auth/scene/del', { idArr: idArr }, true)
                .then((res) => {
                    getList()
                })
                .catch(() => {})
