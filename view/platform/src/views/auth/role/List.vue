...
const handleDelete = (idArr: number[]) => {
	ElMessageBox.confirm('', {
		...
		request(t('config.VITE_HTTP_API_PREFIX') + '/auth/role/del', { idArr: idArr }, true).then((res) => {
			getList()
		}).catch(() => { })
	}).catch(() => { })
}
	...
	<ElRow class="main-table-tool">
		...
		<ElCol :span="8" style="text-align: right;">
			<ElSpace :size="10" style="height: 100%;">
				<MyExportButton :headerList="table.columns" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/auth/role/list', param: { filter: queryCommon.data, sort: table.sort.key + ' ' + table.sort.order } }" />
