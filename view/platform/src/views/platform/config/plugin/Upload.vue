...
const saveForm = reactive({
    ...
    rules: {
        ...
        uploadOfAliyunOssAccessKeyId: [{ type: 'string', trigger: 'blur', pattern: /^[\p{L}\p{N}_-]+$/u, message: t('validation.alpha_dash') }],
        uploadOfAliyunOssAccessKeySecret: [{ type: 'string', trigger: 'blur', pattern: /^[\p{L}\p{N}_-]+$/u, message: t('validation.alpha_dash') }],
    ...
    <el-form :ref="(el: any) => saveForm.ref = el" :model="saveForm.data" :rules="saveForm.rules" label-width="auto" :status-icon="true" :scroll-to-error="false">
        <el-form-item :label="t('platform.config.plugin.name.uploadType')" prop="uploadType">
            <el-radio-group v-model="saveForm.data.uploadType">
                <el-radio v-for="(item, index) in tm('platform.config.plugin.status.uploadType') as any" :key="index" :value="item.value">
                    {{ item.label }}
                </el-radio>
            </el-radio-group>
        </el-form-item>
        ...
        <el-form-item>
            <el-button v-if="authAction.isSave || authAction.isUploadSave" type="primary" @click="saveForm.submit" :loading="saveForm.loading"> <autoicon-ep-circle-check />{{ t('common.save') }} </el-button>
